...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, "public/avatar/"); // Lưu vào thư mục "uploads"
  },
...
router.get("/me", check_authentication, async function (req, res, next) {
  try {
    let user = await userController.GetUserByID(req.user);
...
router.post(
  ...
  async function (req, res, next) {
    try {
      ...
      let avatarUrl = null;
      if (file && file.avatar) {
        avatarUrl = `/public/avatar/${file.avatar.filename}`;
      } else {
        avatarUrl = "/public/avatar/default-avatar.jpg";
      }
      let body = req.body;
